...
function FriendList() {
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      const res = await axios.get(
        `${process.env.REACT_APP_BASE_URL}/api/friend/friend-list`,
        {
          headers: {
  ...
  const handleRemoveFriend = async (receiverId) => {
    try {
      setIsloading(true);

      const res = await axios.delete(
        `${process.env.REACT_APP_BASE_URL}/api/friend/remove-friend/${receiverId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`, // Add token to the Authorization header
          },
                ...
                <div key={index} className="col-12 col-sm-6 col-md-4 col-lg-3">
                  <div className="card w-100" style={{ height: "300px" }}>
                    ...
                    <div className="card-body text-center">
                      ...
                      <button onClick={() => handleRemoveFriend(user?.friend?._id)} className="btn btn-outline-danger">Remove Friend</button>
                    </div>
                  </div>
                </div>
              ))}
